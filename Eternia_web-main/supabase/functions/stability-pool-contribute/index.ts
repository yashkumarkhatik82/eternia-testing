...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Monthly: deduct 1 ECC from each student and add to their institution's stability pool
    ...
    for (const [instId, amount] of Object.entries(poolUpdates)) {
      const { data: currentBalance } = await supabase.rpc("get_pool_balance", { _institution_id: instId });
      await supabase
        .from("ecc_stability_pool")
        .upsert({
          institution_id: instId,
          balance: (currentBalance || 0) + amount,
          total_contributed: amount, // will be incremented
